...
* 一个帐户创建一个新列表。
* 一个帐户在他们的列表中创建了一个新任务。每当有人创建新任务时，都会发出 `task_created` 事件。
* 让一个帐户将他们的任务标记为已完成。
...
* An account creates a new list.
* An account creates a new task on their list. Whenever someone creates a new task, emit a `task_created` event.
* Let an account mark their task as completed. -->
...
> [看看这个](https://aptos.dev/tutorials/build-e2e-dapp/create-a-smart-contract)了解有关此合约的更多信息。
<!-- {% hint style="info" %} -->
<!-- [Check this](https://aptos.dev/tutorials/build-e2e-dapp/create-a-smart-contract) out for more information on this contract. -->
<!-- {% endhint %} -->
