...
- 有两个资源 `CandidateList` 和 `VoterList`，存储在 `my_addrx`（该模块的所有者）下。
- `initialize_with_candidate` 函数由所有者调用以初始化并添加新的候选人进行选举。
- 所有者调用 `add_candidate` 函数来添加更多候选人。
- `vote` 只能被任何人调用一次
- 调用 `declare_winner` 函数声明结果
